...
class DADA(PreTrainedModel):
    ...
    def forward(self, x, grl=0, revin=0):
        output =  self.model.forward(x, grl=grl, revin=revin)
        return output
    
    def infer(self, x, anomaly_criterion=None, revin=0):
        out_copies = self.model.infer(x, revin=revin)
        if anomaly_criterion is None: anomaly_score = 1
        else: anomaly_score = 0
        score = self.model.cal_anomaly_score(batch_x=x, batch_out_copies=out_copies, anomaly_criterion=anomaly_criterion, anomaly_score=anomaly_score)
        return score
...
class Model(nn.Module):
    ...
    def forward(self, x, grl=0, revin=0): # b x t x c
        B, T, dims = x.size()
        # 0.normalization
        if revin:
        ...
        if self.mask_mode=="c":
            mask_1 = torch.from_numpy(np.random.binomial(1, 0.5, size=(B*dims, self.patch_num))).to(torch.bool) # b*c x patch_num
            mask_2 = ~mask_1
            mask = torch.cat([mask_1, mask_2], dim=0) # b*c*2 x patch_num
            input_patch = input_patch.repeat(2, 1, 1) # b*c*2 x patch_num x hidden_dims
            input_patch[mask] = 0 # patch symmetry mask
        else:
            mask = torch.from_numpy(np.random.binomial(1, 0.5, size=(B*dims, self.patch_num))).to(torch.bool) # b*c x patch_num
            input_patch[mask] = 0
        # 4.encoder
        ...
        # 6.symmetry mask: concat masked part
        if self.mask_mode == "c":
    ...
    def infer(self, x, revin=0, mask_mode=None):
    ...
    def cal_anomaly_score(self, batch_x, batch_out_copies, anomaly_criterion=None, anomaly_score=1):
        if anomaly_score==1:
